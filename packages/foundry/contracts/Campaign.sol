...
contract Campaign {
  ...
  modifier isValidGoalAddress(
    uint256 _goalIndex
  ) {
    require(_goalIndex >= 0 && _goalIndex < goals.length, "Invalid goalIndex");
    _;
  }

  modifier isValidAttestation(
    bytes32 uid
  ) {
  ...
  modifier isCampaignNotFinished() {
    require(currentGoalIndex < goals.length, "All goals have been completed");
    _;
  }

  modifier onlyInstitution() {
    require(msg.sender == institutionAddress, "Sender must be institution only");
    _;
  }
  ...
  function fund(
    ...
    // transfer tokens
    require(
      erc20.transferFrom(msg.sender, address(this), _amount), "Transfer failed"
    );
  ...
  function refund(
    uint256 _goalIndex
  ) private isValidGoalAddress(_goalIndex) {
    // find goal
    ...
    for (uint256 i = 0; i < goals[_goalIndex].backers.length; i++) {
      require(
        erc20.transferFrom(
          ...
          goalBackersAndValues[_goalIndex].data[goals[_goalIndex].backers[i]]
        ),
        "Transfer failed"
      );
    }
  ...
  function setGoalAttestation(
    ...
    require(
      goalsAttestationUIDs[_goalIndex].length == 0, "Goal already attested"
    );
